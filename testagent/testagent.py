...
########################################
USE_FAISS = False  # Set to True to enable FAISS caching
...
if USE_FAISS:
    import faiss
    DIMENSION = 384  # Must match the model embedding dimension
...
For location, return a JSON object with keys "city" and "country".  
   - "city": List the specific cities mentioned in the job description, comma-separated (e.g., "New York, Boston").  
   - "country": The country name (e.g., "United States"). If not specified, use an empty string.
...
def parse_resume_data():
    ...
    for _, row in df.iterrows():
        ...
        # Extract location as a structured dictionary.
        ...
        # For skills: robust extraction handling different types.
        ...
        # For education: try "localDescription" first, then fallback to "degreeName"
        education_list = []
...
def multi_value_similarity(jd_list, cand_list, model, faiss_index=None, faiss_cache=None):
    """
    For attributes with multiple values (e.g., skills, certifications):
...
def compute_candidate_score(jd_attrs, cand_attrs, weights, model, faiss_index=None, faiss_cache=None):
    ...
    Special handling:
      ...
      - For "education": use OR logic. If candidate meets at least one required degree's experience threshold,
    ...
    if isinstance(jd_loc, dict) and isinstance(cand_loc, dict):
        ...
        # If city is provided in JD, use city match; otherwise, if only country provided, use that.
        if jd_loc.get("city"):
